...
USE_DOCKER=0
IN_CONTAINER=0
export UNDRABYTE_VM_ONLY=0
export UNDRABYTE_WITH_BLACKARCH=0
export UNDRABYTE_WITH_NVIDIA=0
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --vm-only)        UNDRABYTE_VM_ONLY=1; shift ;;
        --with-blackarch) UNDRABYTE_WITH_BLACKARCH=1; shift ;;
        --with-nvidia)    UNDRABYTE_WITH_NVIDIA=1; shift ;;
    ...
    exec docker run --rm --privileged \
        ...
        bash -c "pacman -Sy --noconfirm --needed archiso git grub edk2-shell && \
                 /repo/scripts/build-iso.sh --in-container \
                   ...
                   $( ((UNDRABYTE_VM_ONLY))        && echo --vm-only ) \
                   $( ((UNDRABYTE_WITH_BLACKARCH)) && echo --with-blackarch ) \
                   $( ((UNDRABYTE_WITH_NVIDIA))    && echo --with-nvidia ) \
...
log "writing packages.x86_64 (${#PACKAGES[@]} packages)"
{
    ...
    printf '# edition=%s desktop=%s vm_only=%s blackarch=%s nvidia=%s version=%s\n\n' \
        "$EDITION" "$DESKTOP" "$UNDRABYTE_VM_ONLY" "$UNDRABYTE_WITH_BLACKARCH" \
        "$UNDRABYTE_WITH_NVIDIA" "$VERSION"
    printf '%s\n' "${PACKAGES[@]}"
} > "$STAGE_DIR/packages.x86_64"

if (( UNDRABYTE_WITH_BLACKARCH )); then
...
install -d "$airootfs/usr/local/share/undrabyte"
cp -a "$PKGDIR" "$airootfs/usr/local/share/undrabyte/packages"
cp -a "$ROOT/docs" "$airootfs/usr/local/share/undrabyte/docs"
install -Dm0644 "$ROOT/README.md" "$airootfs/usr/local/share/undrabyte/README.md"
...
iso="$(find "$OUT_DIR" -maxdepth 1 -name 'undrabyte-*.iso' -printf '%T@ %p\n' \
